...
class Utils {
   public:
	...
	static bool SegmentSphereIntersect(glm::vec3 p1, glm::vec3 p2, glm::vec3 sphere_pos, float sphere_radius) {
		...
		float t1 = (-B - sqdisc);
		if (t0 < 0 && t1 < 0)
			return false; // If both t0, and t1 are negative, the sphere is behind the camera
		if (t0 < sphere_radius || t1 < sphere_radius) { // If t0 or t1 is smaller than the sphere's radius, the segment has intersected the sphere. 
			return true;
		}
		return false;
	}
