...
void MotionPlanner::CreateMotionPlanner() {
	...
	Node* start = new Node();
	start->position = glm::vec3(-10, -10, 5);
	Node* end = new Node();
	end->position = glm::vec3(10, 10, 5);
	pbr.push_back(start);
	pbr.push_back(end);
...
void Connect(Node* n1, Node* n2) {
	if (glm::distance(n1->position, n2->position) < 5) { // Only allow connections that are somewhat close to each other. 
